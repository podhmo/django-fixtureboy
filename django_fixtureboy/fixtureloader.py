...
class XMLToDictHandler(object):
    def __init__(self, stream, contact):
        self.stream = stream
    ...
    def handle_object(self, node):
        ...
        self.contact.on_model_detected(Model)
        data = {}
    ...
    def _get_model_from_node(self, node, attr):
        ...
        return self.contact.get_model(model_identifier)


class FixtureDataIterator(object):
    # todo: supprot zip .etc
    def __init__(self, stream_or_string, contact, handler_factory=XMLToDictHandler):
        if not hasattr(stream_or_string, "read"):
            stream_or_string = StringIO(stream_or_string)
        self.contact = contact
        self.handler = handler_factory(stream_or_string, contact)
